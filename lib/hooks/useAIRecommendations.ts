    ...
    queryFn: async (): Promise<AIRecommendation[]> => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // 如果 API 返回了有效的建议，使用 DeepSeek 的建议
          if (result.recommendations && result.recommendations.length > 0 && !result.fallback) {
            ...
            return convertToAIRecommendations(result as DeepSeekAnalysisOutput)
          }
        }
      } catch (error) {
        console.warn('DeepSeek API 调用失败，使用本地规则:', error)
      }
      
      // 降级方案：使用本地规则生成建议
      console.log('⚠️ 使用本地规则生成建议（降级方案）')
      return generateLocalRecommendations(wardrobeId)
    },
    enabled: !!wardrobeId,
 ...
 * 本地规则生成建议（降级方案）
 */
async function generateLocalRecommendations(wardrobeId: string): Promise<AIRecommendation[]> {
